...
async def create_charity_project(
    ...
    return await investation(new_charity_project, Donation, session)
...
async def partially_update_charity_project(
    ...
    if obj_in.full_amount is not None:
        await check_full_amount_value(obj_in.full_amount,
                                      charity_project.invested_amount)
    updated_charity_project = await charity_project_crud.update(
        charity_project, obj_in, session)
    return await investation(updated_charity_project, Donation, session)
